...
def converter_cancervar(input_cancervar, output_dir, id_amostra):
    ...
    for _, row in df.iterrows():
        ...
        dados_variante = {
            ...
            'Ref': ref_val, 
            'Alt': alt_val,
            'Gene.refGene': row.get('Ref.Gene', row.get('Gene.refGene', '.')), 
            'Func.refGene': func_ref,
            ...
            'gnomAD_genome_ALL': row.get('Freq_gnomAD_genome_ALL', '.'),
            'gnomAD_genome_AFR': extrair_subpopulacao(pops_gnomad, 'AFR'), 
            'gnomAD_genome_AMR': extrair_subpopulacao(pops_gnomad, 'AMR'),
            'gnomAD_genome_ASJ': extrair_subpopulacao(pops_gnomad, 'ASJ'), 
            'gnomAD_genome_EAS': extrair_subpopulacao(pops_gnomad, 'EAS'),
            'gnomAD_genome_FIN': extrair_subpopulacao(pops_gnomad, 'FIN'), 
            'gnomAD_genome_NFE': extrair_subpopulacao(pops_gnomad, 'NFE'),
            'gnomAD_genome_OTH': extrair_subpopulacao(pops_gnomad, 'OTH'),
            'cosmic104': row.get('cosmic91', row.get('dbnsfp30a_cosmic91', '.')), 
            'icgc28': row.get('icgc28', row.get('dbnsfp30a_icgc28', '.')),
            'CADD_raw': row.get('dbnsfp30a_CADD_raw', row.get('CADD_raw', '.')), 
            'CADD_phred': row.get('dbnsfp30a_CADD_phred', row.get('CADD_phred', '.')),
            'MetaLR_score': row.get('dbnsfp30a_MetaLR_score', row.get('MetaLR_score', '.')), 
            'MetaLR_pred': row.get('MetaLR_pred', 'MetaLR_pred'),
            'MetaSVM_score': row.get('dbnsfp30a_MetaSVM_score', row.get('MetaSVM_score', '.')),
            'MetaSVM_pred': row.get('MetaSVM_pred', 'MetaSVM_pred'), 
            'FATHMM_score': row.get('dbnsfp30a_FATHMM_score', row.get('FATHMM_score', '.')),
            'FATHMM_pred': row.get('FATHMM_pred', 'FATHMM_pred'),
            'fathmm-MKL_coding_score': row.get('dbnsfp30a_fathmm-MKL_coding_score', row.get('fathmm-MKL_coding_score', '.')),
            'fathmm-MKL_coding_pred': row.get('fathmm-MKL_coding_pred', 'fathmm-MKL_coding_pred'),             
            'MutationTaster_score': row.get('dbnsfp30a_MutationTaster_score', row.get('MutationTaster_score', '.')),
